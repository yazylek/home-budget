...
export function AppSidebar() {
  ...
  return (
    <Sidebar
      collapsible="icon"
      className="xl:flex z-50 ml-4 h-[95%] mt-4 rounded-2xl bg-[#F8F8F8]  overflow-hidden"
    >
              ...
              {items.map((item) => {
                ...
                return (
                  <SidebarMenuItem key={item.title}>
                    <SidebarMenuButton
                      asChild
                      className={`mb-2 ${isActive ? "bg-[#ececec]" : ""}`}
                    >
                      <a href={item.url}>
                        <item.icon />
        ...
        <Menubar className="p-6">
          <MenubarMenu>
            ...
            <MenubarContent className="bg-white rounded-md w-full min-w-[160px] pb-1">
              ...
              <MenubarItem
                onClick={logout}
                className="w-full text-left px-4 py-2 hover:bg-[#e6f0ff] cursor-pointer"
              >
                <LogOutIcon />
                Logout
              </MenubarItem>
